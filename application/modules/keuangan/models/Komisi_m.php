...
class Komisi_m extends CI_Model {
	...
	public function simpan_edit(){
		...
		$this->db->where('md5(id)',$this->input->post('id'))->update('komisi',
		[
			'tanggal' => convert_date_to_en($this->input->post('tanggal')),
			'nama' => $this->input->post('nama'),
			'jumlah' => $this->input->post('jumlah'),
	...
	public function hapus(){
		$this->db
		->where(['md5(u.id)' => $this->input->post('id')]);
		$this->db->delete('komisi u');
		return $this->db->affected_rows(); 
	}

    var $table = '( select * from komisi ) u';
	var $column_order = array('','u.id'); //set order berdasarkan field yang di mau
	var $column_search = array('u.nama','keterangan'); //set field yang bisa di search
