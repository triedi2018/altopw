...
class Data_karyawan_m extends CI_Model {
	...
	public function list_atasan_edit($divisi){
		return $this->db->get_where('data_karyawan',['divisi' => $divisi])->result_array();
	}
	...
	public function simpan_edit(){
		if ($this->input->post('id') != hash_id($this->input->post('nik'))){
			return json_encode(['status' => 'error','pesan' => 'Jangan yang aneh aneh, dosa lho..']);
		}
		$this->db->where('nik',$this->input->post('nik'))->update('users',['level_user' => $this->input->post('level_user')]);
		$this->db->where('nik',$this->input->post('nik'))->update('data_karyawan',
